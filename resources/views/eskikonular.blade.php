        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            <div class="d-xl-flex justify-content-between align-items-start">
              <h2 class="text-dark font-weight-bold mb-2"> Konu Önerileri </h2>

            </div>
            <hr>
            <div class="row">
              <div class="col-md-12">
                <h3 class="text-dark font-weight-bold mb-2"> Beklemede Olan Konu Önerileri </h3>
                <table class="styled-table">
                    ...
                    <tbody>
                        <tr class="active-row">
                            @forelse ($numara  as $key => $item)
                            @forelse ($konular  as $key => $itemi)
                            <td>{{$itemi->ogrencinumara}} </td>
                            <td>{{$itemi->konu}} </td>
                            <td>{{$itemi->baslık}} </td>
                            <td>{{$itemi->anahtar1}} </td>
                            <td>{{$itemi->materyel}} </td>
                            <td>{{$itemi->onay}} </td>
                ...
                <h3 class="text-dark font-weight-bold mb-2"> Onaylı Konu Önerileri </h3>
                <table class="styled-table">
                    <thead>
                        <tr>
                            <td>Öğrenci Numarası</th>
                    ...
                    <tbody>
                        <tr class="active-row">
                            @forelse ($numara  as $key => $item)
                            @forelse ($konulardort  as $key => $itemi)
                            <td>{{$itemi->ogrencinumara}} </td>
                            <td>{{$itemi->konu}} </td>
                            <td>{{$itemi->baslık}} </td>
                            <td>{{$itemi->anahtar1}} </td>
                ...
                <h3 class="text-dark font-weight-bold mb-2">Reddedilen Konu Önerileri </h3>
                <table class="styled-table">
                    <thead>
                        <tr>
                            <td>Öğrenci Numarası</th>
                    ...
                    <tbody>
                        <tr class="active-row">
                            @forelse ($numara  as $key => $item)
                            @forelse ($konulariki  as $key => $itemi)
                            <td>{{$itemi->ogrencinumara}} </td>
                            <td>{{$itemi->konu}} </td>
                            <td>{{$itemi->baslık}} </td>
                            <td>{{$itemi->anahtar1}} </td>
                ...
                <h3 class="text-dark font-weight-bold mb-2"> Düzenlenmesi İstenilen Konu Önerileri </h3>
                <table class="styled-table">
                    <thead>
                        <tr>
                            <td>Öğrenci Numarası</th>
                    ...
                    <tbody>
                        <tr class="active-row">
                            @forelse ($numara  as $key => $item)
                            @forelse ($konularüc  as $key => $itemi)
                            <td>{{$itemi->ogrencinumara}} </td>
                            <td>{{$itemi->konu}} </td>
                            <td>{{$itemi->baslık}} </td>
                            <td>{{$itemi->anahtar1}} </td>
                            ...
                            <td>{{$itemi->onay}} </td>
                            <td>{{$itemi->acıklama}} </td>
                            <td><a style="color: red" href={{url('düzenlenmeisteği/'.$item->numara.'/'.$itemi->id)}}>
                                Düzenle
                            </a></td>
